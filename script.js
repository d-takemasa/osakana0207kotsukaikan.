document.addEventListener('DOMContentLoaded', () => {
    ...
    const prizes = [
        { id: 'A', name: "A賞：オリジナル カトラリー", image: "prize_images/prize_A.png", weight: 50, maxOccurrences: 200, currentOccurrences: 0, description: "おめでとう！波のきらめきを食卓へ！" },
        { id: 'B', name: "B賞：オリジナル 巾着", image: "prize_images/prize_B.png", weight: 20, maxOccurrences: 75, currentOccurrences: 0, description: "大漁旗のように、あなたの個性をアピールしよう！" },
    ...
    function resetReels() {
        ...
        reel2.innerHTML = '';
        reel3.innerHTML = '';
        resultMessage.innerHTML = '<p>スタートボタンを押してね</p>';
        resultMessage.classList.remove('message-win', 'message-lose');
    }
    ...
    function checkResult(finalResults) {
        if (finalResults[0].id === finalResults[1].id && finalResults[1].id === finalResults[2].id) {
            resultMessage.innerHTML = `<p>スタートボタンを押してね</p>`; // 簡易メッセージ
